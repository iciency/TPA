...
def handler(plugin, sender, args):
    ...
    plugin.tpa_requests[target.unique_id][player.unique_id] = (time.time(), "tpthere")
    ...
    form = MessageForm(
        ...
        content=plugin._(target, "tpa.form.content.tpthere", player.name, return_string=True),
        button1=plugin._(target, "tpa.form.accept", return_string=True),
        button2=plugin._(target, "tpa.form.deny", return_string=True),
    )
    
    plugin._(target, "tpthere.request_received", player.name)
